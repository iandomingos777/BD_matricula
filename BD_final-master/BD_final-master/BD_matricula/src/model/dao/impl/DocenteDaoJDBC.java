...
public class DocenteDaoJDBC implements DocenteDao {
	...
	public void deleteById(int id) throws SQLException {
		String deleteFromCadeira = "DELETE FROM cadeira_aluno WHERE id_professor = ?";
		try (PreparedStatement st = conn.prepareStatement(deleteFromCadeira)) {
			st.setInt(1, id);
			st.executeUpdate();
		}
		
		String unsetCoordenador = "UPDATE curso SET id_coordenador = NULL WHERE id_coordenador = ?";
		try (PreparedStatement st = conn.prepareStatement(unsetCoordenador)) {
			st.setInt(1, id);
			st.executeUpdate();
		}
