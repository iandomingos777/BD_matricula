...
public class DocenteDaoJDBC implements DocenteDao {
	...
	@Override
	public List<Docente> listarANYMenorCarga() throws SQLException {
	    String sql = "SELECT DISTINCT d.* " +
	                 "FROM docente d " +
	                 "JOIN cadeira_aluno ca ON d.id = ca.id_professor " +
	                 "JOIN disciplina di ON ca.id_disciplina = di.id " +
	                 "WHERE di.horas < ANY ( " +
	                 "    SELECT di2.horas " +
	                 "    FROM disciplina di2 " +
	                 "    JOIN cadeira_aluno ca2 ON di2.id = ca2.id_disciplina " +
	                 "    WHERE ca2.id_professor <> d.id " +
	                 ")";
	    try (PreparedStatement st = conn.prepareStatement(sql);
