...
public class CadeiraAlunoDaoJDBC implements CadeiraAlunoDao {
    ...
    @Override
    public void insert(Cadeira_aluno obj) throws SQLException {
    ...
    @Override
    public Cadeira_aluno findById(int id) throws SQLException {
    ...
    @Override
    public List<Cadeira_aluno> findAll() throws SQLException {
        String sql = "SELECT * FROM cadeira_aluno ORDER BY id";

        try (PreparedStatement st = conn.prepareStatement(sql);
             ResultSet rs = st.executeQuery()) {

            List<Cadeira_aluno> list = new ArrayList<>();
            while (rs.next()) {
    ...
    private Cadeira_aluno instantiateCadeiraAluno(ResultSet rs) throws SQLException {
        int id = rs.getInt("id");
        ...
        return new Cadeira_aluno(id, horario, sala, nota1, nota2, nota3, nota4, mat_aluno, id_professor, id_disciplina);
    }
}
